...
export class SupabaseIncidenciasRepository implements IncidenciasPort {
  ...
  private fetchIncidencias(managerId: string | undefined): Observable<IncidenciaEntity[]> {
    ...
    const vacationsQ = this.supabase.rpc('get_pending_vacation_requests_for_reviewer');

    return forkJoin([from(fichajeQ), from(profilesQ), from(dailyQ), from(vacationsQ)]).pipe(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      map(([fichajeRes, profilesRes, dailyRes, vacationsRes]: any[]) => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const vacations: any[] = vacationsRes.data ?? [];
        for (const row of vacations) {
          const fullName = String(row.user_name ?? '').trim();
          if (!fullName) continue;

          incidents.push({
            id: `${row.user_id}::vacation-pending::${row.id}`,
            userId: row.user_id,
            fullName,
