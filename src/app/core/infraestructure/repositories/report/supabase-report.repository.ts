...
export class SupabaseReportRepository implements ReportPort {
  ...
  getMonthlyReport(userId: string, month: number, year: number): Observable<DailyReport[]> {
    // month is 0-indexed from JS Date.getMonth(); SQL dates need 1-indexed month
    const sqlMonth = month + 1;
    const from_date = `${year}-${String(sqlMonth).padStart(2, '0')}-01`;
    const lastDay = new Date(year, sqlMonth, 0).getDate();
    const to_date = `${year}-${String(sqlMonth).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
