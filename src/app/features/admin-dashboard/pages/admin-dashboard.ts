    ...
    this.fichajeApproval.approve(event.id, managerId, event.comment || undefined)
      ...
      .subscribe({
        next: () => {
          ...
          this.alertService.success('Aprobado', 'El fichaje ha sido aprobado.');
        },
        error: () => this.alertService.warning('Error', 'No se pudo aprobar el fichaje.'),
      });
  }
  ...
  onFichajeRejected(event: { id: string; reason: string }): void {
    ...
    this.fichajeApproval.reject(event.id, managerId, event.reason)
      ...
      .subscribe({
        next: () => {
          this.pendingFichajes.update(items => items.filter(f => f.id !== event.id));
          this.alertService.warning('Rechazado', 'El fichaje ha sido rechazado.');
        },
        error: () => this.alertService.warning('Error', 'No se pudo rechazar el fichaje.'),
      });
  }
