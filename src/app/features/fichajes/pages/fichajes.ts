...
export class FichajesPage {
  ...
  readonly texts = computed(() => this.i18n.translations().fichajes);
  ...
  formattedSelectedDate = computed(() => {
    ...
    return date.toLocaleDateString(this.i18n.locale() === 'en' ? 'en-GB' : 'es-ES', options);
  });
  ...
  /** Called when user clicks an action CTA directly — sets type then submits */
  ...
  addEntry(): void {
    ...
    this.fichajeFlow
      ...
      .subscribe({
        ...
        error: () => {
          this.saving.set(false);
          this.appAlertService.warning(this.texts().alerts.saveErrorTitle, this.texts().alerts.saveErrorMessage);
        },
      });
  }
